...
const { mungeResponse } = require('./munge-functions.js');
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    const formattedResponse = mungeResponse(searchURL.body);
...
app.delete('/api/favorites/:id', async (req, res) => {
  try {
    const data = await client.query('DELETE from favorites WHERE user_id=$1 AND id=$2 RETURNING *', [req.userId, req.params.id],);
